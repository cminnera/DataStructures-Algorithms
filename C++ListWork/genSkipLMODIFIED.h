...
T* SkipList<T>::skipListSearch(const T& key) {
    ...
    while (1) {
        if (key == curr->key)                   // success if equal;
             return &curr->key;                 //   return what matched
...
template<class T>
T* SkipList<T>::skipListInsert(const T& key) {
    ...
    for (lvl = maxLevel - 1; lvl >= 0; lvl--) {
		...
		if (curr[lvl] && curr[lvl]->key == key)    // don't include
			// MODIFED TO RETURN A POINTER TO WHAT MATCHED
			//return;                                // duplicates;
			return &(curr[lvl]->key);
